...
int solve(string A) {
    ...
    */
    ...
    // Because current appending "A then original"
    // indicates that all the character are matched and unmatched
    // characters can be inserted at begining of A(reversed string )
    // which is just inserting the elements at end of original string
    A = A+"$"+original;
